 ...
 *
 * @generated from core_blas/core_ztsmqr.c, normal z -> s, Mon Mar 18 06:35:39 2024
 *
 **/
 ...
 *    Q = H(1) H(2) . . . H(k)
 *
 *  as returned by coreblas_stsqrt.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *         coreblas_STSQRT in the first k columns of its array argument V.
 *
 * @param[in] ldv
                      ...
                      float *work, int ldwork)
{
    ...
    for (int i = i1; i > -1 && i < k; i += i3) {
        ...
        // Apply H or H^T (NOTE: coreblas_sparfb used to be core_ztsrfb).
        coreblas_sparfb(side, trans, CoreBlasForward, CoreBlasColumnwise,
                    mi, ni, m2, n2, kb, 0,
                    &A1[lda1*jc+ic], lda1,
                    A2, lda2,
                    &V[ldv*i], ldv,
                     ...
                     coreblas_workspace_t work)
{
    ...
    // Call the kernel.
    int info = coreblas_stsmqr(side, trans,
                           m1, n1, m2, n2, k, ib,
                           A1, lda1,
                           A2, lda2,
                           V,  ldv,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_stsmqr() failed");
    }
}
