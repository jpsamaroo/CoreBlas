 ...
 *
 * @generated from core_blas/core_ztsmlq.c, normal z -> c, Mon Mar 18 06:35:39 2024
 *
 **/
 ...
 *    Q = H(k)^H . . . H(2)^H H(1)^H
 *
 *  as returned by coreblas_ctslqt.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *         coreblas_ctslqt in the first k rows of its array argument V.
 *
 * @param[in] ldv
                      ...
                      coreblas_complex32_t *work, int ldwork)
{
    ...
    for (int i = i1; i > -1 && i < k; i += i3) {
        ...
        // Apply H or H^H.
        coreblas_cparfb(side, trans, CoreBlasForward, CoreBlasRowwise,
                    mi, ni, m2, n2, kb, 0,
                    &A1[lda1*jc+ic], lda1,
                    A2, lda2,
                    &V[i], ldv,
                     ...
                     coreblas_workspace_t work)
{
    ...
    int ldwork = side == CoreBlasLeft ? ib : n1;  // TODO: float check
    // Call the kernel.
    int info = coreblas_ctsmlq(side, trans,
                           m1, n1, m2, n2, k, ib,
                           A1, lda1,
                           A2, lda2,
                           V,  ldv,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_ctsmlq() failed");
    }
