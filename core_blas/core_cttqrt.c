 ...
 *
 * @generated from core_blas/core_zttqrt.c, normal z -> c, Mon Mar 18 06:35:40 2024
 *
 **/
                ...
                coreblas_complex32_t *work)
{
    ...
    //core_claset(CoreBlasGeneral, ib, n, 0.0, 0.0, T, ldt);
    ...
    for (int ii = 0; ii < n; ii += ib) {
        ...
        for (int i = 0; i < sb; i++) {
            ...
            if (ni > 0) {
                ...
                LAPACKE_clacgv_work(ni, work, 1);
#endif
                coreblas_complex32_t alpha = -conjf(tau[j]);
                cblas_caxpy(
            ...
            if (i > 0) {
                ...
                coreblas_complex32_t alpha = -(tau[j]);
                     ...
                     coreblas_workspace_t work)
{
    ...
    // Call the kernel.
    int info = coreblas_cttqrt(m, n, ib,
                           A1, lda1,
                           A2, lda2,
                           T,  ldt,
                           tau,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_cttqrt() failed");
    }
