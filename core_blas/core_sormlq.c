 ...
 *
 * @generated from core_blas/core_zunmlq.c, normal z -> s, Mon Mar 18 06:35:40 2024
 *
 **/
 ...
 *        Q = H(k) . . . H(2) H(1)
 *    \f]
 *  as returned by coreblas_sgelqt. Q is of order m if side = CoreBlasLeft
 ...
 *         coreblas_sgelqt in the first k rows of its array argument A.
 *
 * @param[in] lda
                     ...
                     coreblas_workspace_t work)
{
    ...
    // Call the kernel.
    int info = coreblas_sormlq(side, trans,
                           m, n, k, ib,
                           A, lda,
                           T, ldt,
                           C, ldc,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_sormlq() failed");
    }
