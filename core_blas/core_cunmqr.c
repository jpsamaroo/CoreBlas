 ...
 *
 * @generated from core_blas/core_zunmqr.c, normal z -> c, Mon Mar 18 06:35:41 2024
 *
 **/
 ...
 *        Q = H(1) H(2) ... H(k)
 *    \f]
 *  as returned by coreblas_cgeqrt. Q is of order m if side = CoreBlasLeft
 ...
 *         as returned by coreblas_cgeqrt in the first k columns of its
 *         array argument A.
 *
 * @param[in] lda
                     ...
                     coreblas_workspace_t work)
{
    ...
    // Call the kernel.
    int info = coreblas_cunmqr(side, trans,
                           m, n, k, ib,
                           A, lda,
                           T, ldt,
                           C, ldc,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_cunmqr() failed");
    }
