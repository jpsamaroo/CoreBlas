 ...
 *
 * @generated from core_blas/core_zunmqr.c, normal z -> s, Mon Mar 18 06:35:41 2024
 *
 **/
 ...
 *        Q = H(1) H(2) ... H(k)
 *    \f]
 *  as returned by coreblas_sgeqrt. Q is of order m if side = CoreBlasLeft
 ...
 *         as returned by coreblas_sgeqrt in the first k columns of its
 *         array argument A.
 *
 * @param[in] lda
                     ...
                     coreblas_workspace_t work)
{
    ...
    // Call the kernel.
    int info = coreblas_sormqr(side, trans,
                           m, n, k, ib,
                           A, lda,
                           T, ldt,
                           C, ldc,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_sormqr() failed");
    }
