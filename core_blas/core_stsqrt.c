 ...
 *
 * @generated from core_blas/core_ztsqrt.c, normal z -> s, Mon Mar 18 06:35:39 2024
 *
 **/
                ...
                float *work)
{
    ...
    for (int ii = 0; ii < n; ii += ib) {
...
#endif
                cblas_sgemv(CblasColMajor, (CBLAS_TRANSPOSE)CoreBlasTrans,
                            m, sb-i-1,
        ...
        if (n > ii+sb) {
            coreblas_stsmqr(CoreBlasLeft, CoreBlasTrans,
                     ...
                     coreblas_workspace_t work)
{
    ...
    // Call the kernel.
    int info = coreblas_stsqrt(m, n, ib,
                           A1, lda1,
                           A2, lda2,
                           T,  ldt,
                           tau,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_stsqrt() failed");
    }
