 ...
 *    Q = H(k)^H . . . H(2)^H H(1)^H
 *
 *  as returned by coreblas_zttlqt.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *         coreblas_zttlqt in the first k rows of its array argument V.
 *
 * @param[in] ldv
                     ...
                     coreblas_complex64_t *work, int ldwork)
{
    ...
    // Call the kernel.
    int info = coreblas_zttmlq(side, trans,
                           m1, n1, m2, n2, k, ib,
                           A1, lda1,
                           A2, lda2,
                           V,  ldv,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_zttmlq() failed");
    }
}
