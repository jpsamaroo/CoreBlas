 ...
 *
 * @generated from core_blas/core_zunmlq.c, normal z -> c, Mon Mar 18 06:35:40 2024
 *
 **/
 ...
 *        Q = H(k) . . . H(2) H(1)
 *    \f]
 *  as returned by coreblas_cgelqt. Q is of order m if side = CoreBlasLeft
 ...
 *         coreblas_cgelqt in the first k rows of its array argument A.
 *
 * @param[in] lda
                     ...
                     coreblas_workspace_t work)
{
    ...
    int ldwork = side == CoreBlasLeft ? n : m; // TODO: float check 
    // Call the kernel.
    int info = coreblas_cunmlq(side, trans,
                           m, n, k, ib,
                           A, lda,
                           T, ldt,
                           C, ldc,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_cunmlq() failed");
    }
