 ...
 *
 * @generated from core_blas/core_ztsmqr.c, normal z -> c, Mon Mar 18 06:35:39 2024
 *
 **/
 ...
 *    Q = H(1) H(2) . . . H(k)
 *
 *  as returned by coreblas_ctsqrt.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *         coreblas_CTSQRT in the first k columns of its array argument V.
 *
 * @param[in] ldv
                      ...
                      coreblas_complex32_t *work, int ldwork)
{
    ...
    for (int i = i1; i > -1 && i < k; i += i3) {
        ...
        // Apply H or H^H (NOTE: coreblas_cparfb used to be core_ztsrfb).
        coreblas_cparfb(side, trans, CoreBlasForward, CoreBlasColumnwise,
                    mi, ni, m2, n2, kb, 0,
                    &A1[lda1*jc+ic], lda1,
                    A2, lda2,
                    &V[ldv*i], ldv,
                     ...
                     coreblas_workspace_t work)
{
    ...
    coreblas_complex32_t *W = (coreblas_complex32_t*)work.spaces[tid];
    int ldwork = side == CoreBlasLeft ? ib : m1; // TODO: float check
    // Call the kernel.
    int info = coreblas_ctsmqr(side, trans,
                           m1, n1, m2, n2, k, ib,
                           A1, lda1,
                           A2, lda2,
                           V,  ldv,
    ...
    if (info != CoreBlasSuccess) {
        coreblas_error("core_ctsmqr() failed");
    }
}
