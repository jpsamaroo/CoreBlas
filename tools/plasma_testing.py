...
###############################################################################
# This testing suite is derived from the coreblas_testing.py script of the COREBLAS
...
# Example:
#     ./coreblas_testing_intertwine.py
...
# set path to COREBLAS testing directory (part of the distribution, not the installed one)
coreblas_dir = ".."
path_to_coreblas_tests = os.path.realpath(coreblas_dir) + "/test"

#print path_to_coreblas_tests
...
os.environ["PATH"] = os.environ["PATH"]+":"+path_to_coreblas_tests

os.environ["COREBLAS_TUNING_FILENAME"] = coreblas_dir + "/tuning/default.lua"
...
print " "
print "----- Testing the subset of double precision COREBLAS Routines related to INTERTWinE project -----"
print " "
...
for dtype in range(4):
   ...
   #binary = path_to_coreblas_tests + "/" + "%stesting" % letter
   binary = path_to_coreblas_tests + "/test" 
   ...
   if (os.path.exists(binary) and os.access(binary,os.X_OK)):
      ...
      print "Have you build COREBLAS by make in the COREBLAS directory:", coreblas_dir, "?"

print "----- Testing the subset of COREBLAS Routines -----"
if (any(errors)):
