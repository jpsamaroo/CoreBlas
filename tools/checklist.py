...
#
# Usage: from top level of coreblas directory:
...
def process( fname ):
    ...
    if (re.search( r'\.(c|cc|cpp|h|hh|hpp)$', filename )):
        ...
        grep( lines, r"'",
              exclude=r"'[a-zA-Z=:+]'|'\\0'|LAPACK's|COREBLAS's|can't",
              help="Might be using Matlab-style A'. Please use A^H or A^T. (Second search.)" )

        grep( lines, r"@ingroup",
              exclude=r'@ingroup (coreblas|core)_[^z]\w+',
              help="Use @ingroup (coreblas|core)_{routine}, without precision: e.g., coreblas_gemm.\n"
